...
%% 
%%     $Id: mnesia_schema.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
...
do_read_table_property(Tab, Key) ->
    Props = find_props(do_read_table_info(Tab)),
...
do_read_table_info(Tab) ->
	...
	true ->
	    case Info of
		[] ->
		    mnesia:abort({no_exists, {Tab,cstruct}});
		I ->
		    I
	    end;
	false ->
	    cs2list(val({Tab, cstruct}))
    end.
...
prepare_op(Tid, {op, add_table_copy, Storage, Node, TabDef}, _WaitFor) ->
	...
	Node == node() ->
		...
		ram_copies when Storage /= ram_copies -> 
		    %% TODO: should we consider external copies here?
...
restore(Opaque, Args, Module) when list(Args), atom(Module) ->
    ...
    case catch lists:foldl(fun check_restore_arg/2, InitR, Args) of
 	R when record(R, r) ->
 		...
 		{error, Reason} ->
		    io:format("mnesia_bup:read_schema() failed: ~p.~n", 
			      [Reason]),
 		    {aborted, Reason};
 		BupSchema -> 
		    io:format("got BupSchema~n",[]),
...
prepare_restore(Opaque, Args, Module) when is_list(Args), is_atom(Module) ->
    io:format("~p: prepare_restore() called~n", [?LINE]),
    InitR = #r{opaque = Opaque, module = Module},
    case catch lists:foldl(fun check_restore_arg/2, InitR, Args) of
	R when record(R, r) ->
	    case mnesia_bup:read_schema(Module, Opaque) of
		{error, Reason} ->
		    exit(Reason);
		BupSchema ->
		    io:format("~p: read_schema() -> ~p~n", [?LINE,BupSchema]),
		    {R, BupSchema}
	    end;
	{'EXIT', Reason} ->
	    io:format("~p: got {'EXIT',~p} - ~p~n", [?LINE, Reason,
						     erlang:get_stacktrace()]),
	    {aborted, Reason}
    end;
prepare_restore(_Opaque, Args, Module) ->
    erlang:error({badarg, Args, Module}).
