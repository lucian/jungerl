...
%% 
%%     $Id: mnesia_controller.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
...
late_disc_load(TabsR, Reason, RemoteLoaders, From, 
    ...
    Tabs = sort_on_load_order(lists:foldl(Filter, [], TabsR)),
...
sort_on_load_order(Tabs) ->
    [{T,R} ||
	{T,R,_} <-
	    lists:reverse(
	      lists:keysort(3, [{Tab,Reason,?catch_val({Tab,load_order})} ||
				   {Tab,Reason} <- Tabs]))].
...
handle_cast({disc_load, Tab, {requeue, Reason}}, State) ->
    io:format("received requeue reques (~p, ~p)~n", [Tab, Reason]),
    Worker = #disc_load{table = Tab, reason = Reason},
    State2 = add_worker(Worker, State),
    noreply(increase_requeue_delay(State2));
	...
	case Done#loader_done.is_loaded of
		...
		case Done#loader_done.needs_reply of
			...
			case Done#loader_done.reply of
				...
				case Rest of 
				    [] ->
					%% avoid going into a busy loop
					Delay = State#state.requeue_delay,
					timer:apply_after(
					  Delay, 
					  gen_server, cast,
					  [?SERVER_NAME, 
...
orphan_tables([Tab | Tabs], Node, Ns, Local, Remote) ->
    ...
    DiscExternal = disc_external_copies(Cs#cstruct.external_copies, Tab),
    DiscCopyHolders = ordsets:from_list(
			(CopyHolders -- RamCopyHolders) ++ DiscExternal),
...
last_consistent_replica(Tab, Downs) ->
    ...
    DiscExternal = disc_external_copies(External, Tab),
    AmRamOnly = (Storage == ram_copies) or ext_ram_only(External, Tab),
...
%%% ext_ram_only(Ext,Tab) ->
%%% - false if no ext copy locally
%%% - false if any local copies is not ram_only
%%% - true otherwise
ext_ram_only([], _Tab) -> false;
ext_ram_only(Ext, Tab) ->
    lists:all(
      fun({Mod,_Ns}) ->
	      Mod:is_ram_only(Tab)
      end, [{M,N} || {M,N} <- Ext, lists:member(node(),N)]).
...
disc_load_table(Tab, Reason, ReplyTo) ->
	...
	Res == {loaded, ok} ->
	    Done#loader_done{needs_announce = true,
			     ...
			     reply = Res};
	Res == requeue ->
	    io:format("Result is requeue"
		      "Done = ~p~n", [Done]),
	    Done#loader_done{is_loaded = false,
			     reply = {requeue, Reason}};
	ReplyTo /= undefined ->
	    Done#loader_done{is_loaded = false,
			     reply = Res};
	true ->
...
increase_requeue_delay(#state{requeue_delay = Prev} = State) ->
    State#state{requeue_delay = round(Prev * 1.5)}.

disc_external_copies(External, Tab) ->
    ordsets:from_list(
      lists:concat([N || {M,N} <- External,
			 not(M:is_ram_only(Tab))])).
