...
%% 
%%     $Id: mnesia_loader.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
...
do_get_disc_copy2(Tab, _Reason, Storage, _Type) when Storage == unknown ->
    verbose("Local table copy of ~p has recently been deleted, ignored.~n",
	    [Tab]),
    {loaded, ok};  %% ?
do_get_disc_copy2(Tab, Reason, {external_copies,Mod}, Type) ->
	...
	case Reason of
		...
		ok;
	    _ ->
		case mnesia:onload_fun(Tab, Reason) of
		    UserErr = {error,_} ->
			UserErr;
		    Requeue = requeue ->
			Requeue;
		    OnLoadFun when is_function(OnLoadFun, 1) ->
			Mod:load_table(Tab, Reason, Repair, Type, OnLoadFun)
		end
	end,
    case Res of
	ok ->
...
do_get_disc_copy2(Tab, Reason, Storage, Type) when Storage == disc_copies ->
	...
	case Reason of 
		...
		case mnesia:onload_fun(Tab, Reason) of
			...
			Requeue;
		    OnLoadFun when is_function(OnLoadFun, 1) ->
			io:format("OnloadFun (~p) is valid~n",
				  [OnLoadFun]),
...
dets_to_ets(Tab, Tab, Type, Repair, no, OnLoadFun) ->
    wrap_load(
      OnLoadFun,
      fun() ->
	      mnesia_lib:dets_to_ets(Tab, Tab, Type, Repair, no, OnLoadFun)
      end).
...
do_get_network_copy(Tab, Reason, Ns, Storage, Cs) ->    
	...
	true ->
		...
		ok ->
		    ...
		    {loaded, ok};
		{error, Err} when element(1, Reason) == dumper ->
		    {not_loaded,Err};
		Again when Again == restart; Again == requeue ->
...
%%%		OnLoadFun = mnesia:onload_fun(Tab, Cs, Reason),
		case mnesia:onload_fun(Tab, Cs, Reason) of
		    UserErr = {error, _} ->
			UserErr;
		    Requeue = requeue ->
			Requeue;
		    OnLoadFun when is_function(OnLoadFun, 1) ->
			Init = table_init_fun(SenderPid, OnLoadFun),
%%% 			Args = [self(),Tab,Storage,Cs,SenderPid,TabSize,
%%% 				DetsData,Init],
			ReplyTo = self(),
			Pid = spawn_link(
				fun() ->
...
table_init_fun(SenderPid, OnLoadFun) ->
    ...
    fun(read) ->
	    ...
	    get_data(SenderPid, Receiver, OnLoadFun)
    end.


%% Add_table_copy get's it's own locks.
start_receiver(Tab,Storage,Cs,SenderPid,TabSize,DetsData,
	       {dumper,add_table_copy} = LoadReason) ->
    Res =
	case mnesia:onload_fun(Tab, LoadReason) of
	    UserErr = {error, _} ->
		UserErr;
	    Requeue = requeue ->
		Requeue;
...
make_table_fun(Pid, TabRec, OnLoadFun) ->
    fun(close) ->
	    ok;
       (read) ->
	    get_data(Pid, TabRec, OnLoadFun)
    end.
...
get_data(Pid, TabRec, OnLoadFun) ->
	...
	{Pid, {more, Recs}} ->
	    ...
	    Pid ! {TabRec, more},
	    {Recs, make_table_fun(Pid,TabRec, OnLoadFun)};
	{Pid, no_more} ->
	    end_of_input;
	{copier_done, Node} ->
	    case node(Pid) of
...
update_where_to_write(Tab, Node) ->
	...
	read_only -> 
	    ignore;
	RW when RW==read_write; RW==index -> 
