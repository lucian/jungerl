...
%% 
%%     $Id: mnesia.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
...
raw_table_info(Tab, Item) ->
	...
	{external_copies, Mod} ->
	    info_reply(catch Mod:table_info(Tab, Item), Tab, Item);
	unknown ->
	    bad_info_reply(Tab, Item);
	{'EXIT', _} ->
	    bad_info_reply(Tab, Item)
...
%%% This function is called by mnesia_loader. It can be shadowed by
%%% another access module. For backward compatibility and safety reasons,
%%% the function is optional in user-provided access modules
%%% (basically, if you don't know what you're doing, you shouldn't attempt
%%% to modify this function, since a crash will cause the mnesia node to
%%% dump core.)

onload_fun(Table, LoadReason) ->
    Default = fun(_) -> ok end,
    case system_info(access_module) of
	?MODULE ->
	    Default;
	OtherMod ->
	    try OtherMod:onload_fun(Table, LoadReason)
	    catch
		error:undef ->
		    Default;
		  error:Other ->
		    mnesia_lib:coredump({Other, [{OtherMod,
						  onload_fun,
						  [Table,LoadReason]},
