...
%% 
%%     $Id: mnesia_tm.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
-module(mnesia_tm).
...
execute_outer(Mod, Fun, Args, Factor, Retries, Type) ->
	...
	{new_tid, Tid, Store} ->
	    ...
	    execute_transaction(Fun, Args, Factor, Retries, Type, NewTidTs)
    end.

begin_activity(Type, Mod) when Type==async; Type==sync->
    case req(start_outer) of
	{error, Reason} -> 
	    {aborted, Reason};
	{new_tid, Tid, Store} ->
	    ...
	    NewTidTs = {Mod, Tid, Ts},
	    put(mnesia_activity_state, NewTidTs),
	    try Mod:begin_activity(Type, _Factor=1, Tid, Ts)
	    catch
		error:undef ->
		    ok;
		  error:Other ->
...
execute_transaction(Fun, Args, Factor, Retries, Type, TidTs) ->
    try apply_fun(Fun, Args, Factor, Type, TidTs) of
	{atomic, _Value} = Result ->
	    ...
	    flush_downs(),
	    %% end_activity/2 has already been called
	    Result;
	{nested_atomic, _Value} = Result ->
	    mnesia_lib:incr_counter(trans_commits),
%%	    end_activity(Result, TidTs),
	    Result
    catch
	error:Reason ->
	  ...
	  exit:Reason ->
	    check_exit(Fun, Args, Factor, Retries, Reason, Type);
	  throw:Value -> %% User called throw
	    Reason = {aborted, {throw, Value}},
	    return_abort(Fun, Args, Type, Reason)
    end.

end_activity(Result, Type, {Mod,Tid,Ts}) ->
    try Mod:end_activity(Result, Type, Tid, Ts)
    catch
	error:undef ->
	    ok;
	  error:Other ->
	    mnesia_lib:coredump({'EXIT',{Other, erlang:get_stacktrace()}})
    end.

apply_fun(Fun, Args, Factor, Type, TidTs) ->
    {Mod,Tid,Ts} = TidTs,
    try Mod:begin_activity(Type, Factor, Tid, Ts)
    catch
	error:undef ->
	    ok;
	  error:Other ->
    ...
    commit_activity(Result, Type, TidTs).

abort_activity(Reason, Type) ->
    check_exit(no_restart, [], _Factor=1, _Retries=0, Reason, Type).

commit_activity(Result, Type, TidTs) ->
    EndActivityCommitted = fun(TType) -> 
				   end_activity({atomic, Result}, TType, TidTs)
			   end,
    case t_commit(Type, EndActivityCommitted) of
	do_commit ->
            {atomic, Result};
        do_commit_nested ->
...
maybe_restart(Fun, Args, Factor, Retries, Type, Why) ->
	...
	yes when Ts#tidstore.level == 1 ->
	    if Fun==no_restart ->
		    return_abort(Fun, Args, Type, Why);
	       true ->
		    restart(
		      Mod, Tid, Ts, Fun, Args, Factor, Retries, Type, Why)
	    end;
	yes ->
	    return_abort(Fun, Args, Type, Why);
	no ->
	    return_abort(Fun, Args, Type, {aborted, nomore})
    end.
...
return_abort(Fun, Args, Type, Reason)  ->
    {_Mod, Tid, Ts} = TidTs = get(mnesia_activity_state),
    end_activity({aborted, Reason}, Type, TidTs),
	...
	Ts#tidstore.level == 1 ->
		...
		{N, Prep} when N > 0 ->
		    multi_commit(Prep#prep.protocol,
				 Tid, Prep#prep.records, Store, EACF);
		{0, Prep} ->
		    multi_commit(read_only, Tid, Prep#prep.records, Store, EACF)
	    end;
	true ->
	    %% nested commit
	    EACF(Type),
...
prepare_schema_items(Tid, Items, Prep) ->
    ...
    Recs = prepare_nodes(Tid, schema, Types, Items, Prep#prep.records, schema),
...
do_prepare_items(Tid, Tab, Key, Types, Snmp, Items, Recs) ->
    ...
    prepare_nodes(Tid, Tab, Types, Items, Recs2, normal).
...
prepare_snmp(Tid, Tab, Key, Types, Us, Items, Recs) -> 
    if Key /= '_' ->
	    ...
	    prepare_nodes(Tid, Tab, Types, [{Op, Tab, Key, SnmpOid}], Recs, snmp);
       Key == '_' ->
	    prepare_nodes(Tid, Tab, Types, [{clear_table, Tab}], Recs, snmp)
    end.
...
prepare_nodes(Tid, Tab, [{Node, Storage} | Rest], Items, C, Kind) ->
    {Rec, C2} = pick_node(Tid, Node, C, []),
    Rec2 = prepare_node(Node, Tab, Storage, Items, Rec, Kind),
    [Rec2 | prepare_nodes(Tid, Tab, Rest, Items, C2, Kind)];
prepare_nodes(_Tid, _Tab, [], _Items, CommitRecords, _Kind) ->
    CommitRecords.
...
prepare_node(Node, Tab, Storage, [Item | Items], Rec, Kind) when Kind == snmp ->
    Rec2 = Rec#commit{snmp = [Item | Rec#commit.snmp]},
    prepare_node(Node, Tab, Storage, Items, Rec2, Kind);
prepare_node(Node, Tab, Storage, [Item | Items], Rec, Kind) when Kind /= schema ->
    Rec2 = 
	case Storage of
	    ram_copies ->
...
multi_commit(sym_trans, Tid, CR, Store, EACF) ->
	...
	do_commit ->
	    mnesia_recover:note_decision(Tid, committed),
	    do_dirty(Tid, Local),
	    EACF(sym_trans),  %end_activity({atomic, Result}, ...)
	    mnesia_locker:release_tid(Tid),
	    ?MODULE ! {delete_transaction, Tid};
	{do_abort, _Reason} ->
	    mnesia_recover:note_decision(Tid, aborted)
...
multi_commit(sync_sym_trans, Tid, CR, Store, EACF) ->
	...
	do_commit ->
	    ...
	    do_commit(Tid, Local),
	    EACF(sync_sym_trans), % end_activity({atomic, Result}, ...)
...
multi_commit(asym_trans, Tid, CR, Store, EACF) ->
	...
	do_commit ->
		...
		{_Modified, C, DumperMode} when record(C, commit) ->
		    ...
		    EACF1 = fun() -> EACF(asym_trans) end,
		    rec_acc_pre_commit(Pids, Tid, Store, EACF1, {C,Local}, 
...
rec_acc_pre_commit([Pid | Tail], Tid, Store, EACF, Commit, Res, DumperMode, 
		   GoodPids, SchemaAckPids) ->
    receive
	{?MODULE, _, {acc_pre_commit, Tid, Pid, true}} ->
	    rec_acc_pre_commit(Tail, Tid, Store, EACF, Commit, Res, DumperMode,
	...
	{?MODULE, _, {do_abort, Tid, Pid, _Reason}} ->
	    ...
	    rec_acc_pre_commit(Tail, Tid, Store, EACF, Commit, AbortRes,
			       DumperMode, GoodPids, SchemaAckPids);
...
rec_acc_pre_commit([], Tid, Store, EACF,
		   {Commit,OrigC}, Res, DumperMode, GoodPids, SchemaAckPids) ->
	...
	do_commit ->
	    ...
	    EACF(), % end_activity({atomic, Result}, ...)
	    mnesia_locker:release_tid(Tid),
	    ?MODULE ! {delete_transaction, Tid};
...
do_commit(Tid, C, DumperMode) ->
    ...
    lists:foldl(
      fun({Mod,Ops}, ResX) ->
	      do_update(Tid, {external_copies,Mod}, Ops, ResX)
      end, R4, C#commit.external_copies).
