...
%% 
%%     $Id: mnesia_lib.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
...
storage_type_at_node(Node, Tab) ->
    search_key(Node, [{disc_copies, val({Tab, disc_copies})},
		      ...
		      [{{external_copies, Mod}, Ns} ||
			  {Mod,Ns} <- val({Tab, external_copies})]]).
...
cs_to_nodes(Cs) ->
	...
	ext_copy_nodes(Cs#cstruct.external_copies).

ext_copy_nodes(Ext) ->
    ordsets:from_list(lists:concat([Ns || {_, Ns} <- Ext])).
...
db_get({external_copies, Mod}, Tab, Key) -> Mod:db_get(Tab, Key).
...
db_select_cont(disc_only_copies, Cont0, Ms) ->
    ...
    dets:select(Cont);
db_select_cont({external_copies, Mod}, Cont0, Ms) ->
...
db_first({external_copies, Mod}, Tab) -> Mod:db_first(Tab).
...
db_update_counter({external_copies, Mod}, Tab, C, Val) ->
    Mod:db_update_counter(Tab, C, Val).
...
%% assuming that Tab is a valid ets-table
dets_to_ets(Tabname, Tab, File, Type, Rep, Lock, OnLoadFun) ->
    {Open, Close} = mkfuns(Lock),
    case Open(Tabname, [{file, File}, {type, disk_type(Tab, Type)},
			{keypos, 2}, {repair, Rep}]) of
	{ok, Tabname} ->
%%%	    Res = dets:to_ets(Tabname, Tab),
	    Res = chunk_dets_to_ets(dets:bchunk(Tabname, start), Tabname,
				    Tab, OnLoadFun),
	    Close(Tabname),
	    trav_ret(Res, Tab);
	Other ->
...
chunk_dets_to_ets('$end_of_table', _Tabname, Tab, _OnLoadFun) ->
    Tab;
chunk_dets_to_ets({error, Reason}, Tabname, _Tab, _OnLoadFun) ->
    {error, {Reason, Tabname}};
chunk_dets_to_ets({Cont, Objs}, Tabname, Tab, OnLoadFun) ->
    ets:insert(Tab, Objs),
    OnLoadFun([{write,O} || O <- Objs]),
    chunk_dets_to_ets(dets:bchunk(Tabname, Cont), Tabname, Tab, OnLoadFun).
