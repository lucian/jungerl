...
%% 
%%     $Id: mnesia_log.erl,v 1.1 2006/03/04 14:39:18 uwiger Exp $
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
...
dcd2ets(Tab, Rep) ->
    dcd2ets(Tab, Rep, fun(_) -> ok end).

dcd2ets(Tab, Rep, OnLoadFun) ->
    Dcd = mnesia_lib:tab2dcd(Tab),
	...
	true ->
	    ...
	    ok = insert_dcdchunk(Data, Log, Tab, OnLoadFun),
	    close_log(Log),
	    load_dcl(Tab, Rep, OnLoadFun); 
...
insert_dcdchunk({Cont, [LogH | Rest]}, Log, Tab, OnLoadFun) 
  when record(LogH, log_header),  
       LogH#log_header.log_kind == dcd_log, 
       LogH#log_header.log_version >= "1.0" ->    
    io:format("log_header~n", []),
    insert_dcdchunk({Cont, Rest}, Log, Tab, OnLoadFun);   

insert_dcdchunk({Cont, Recs}, Log, Tab, OnLoadFun) ->
    io:format("insert_dcdchunk(Recs = ~p)~n", [Recs]),
    true = ets:insert(Tab, Recs),
    OnLoadFun([{write,R} || R <- Recs]),
    insert_dcdchunk(chunk_log(Log, Cont), Log, Tab, OnLoadFun);
insert_dcdchunk(eof, _Log, _Tab, _OnLoadFun) ->
    ok.
...
load_dcl(Tab, Rep, OnLoadFun) ->
	...
	true -> 	    
	    ...
	    FirstChunk = chunk_log(Name, start),
            N = insert_logchunk(FirstChunk, Name, OnLoadFun, 0),
	    close_log(Name),
	    N;
	false ->
	    0
...
insert_logchunk({C2, Recs}, Name, OnLoadFun, C) ->
    N = add_recs(Recs, OnLoadFun, C),
    insert_logchunk(chunk_log(Name, C2), Name, OnLoadFun, C+N);
insert_logchunk(eof, _Name, _OnLoadFun, C) ->
    C.

add_recs([{{Tab, _Key}, Val, write} | Rest], OnLoadFun, N) ->
    true = ets:insert(Tab, Val),
    OnLoadFun([{write, Val}]),
    add_recs(Rest, OnLoadFun, N+1);
add_recs([{{Tab, Key}, Val, delete} | Rest], OnLoadFun, N) ->
    true = ets:delete(Tab, Key),
    OnLoadFun([{delete, Val}]),
    add_recs(Rest, OnLoadFun, N+1);
add_recs([{{Tab, _Key}, Val, delete_object} | Rest], OnLoadFun, N) ->
    true = ets:match_delete(Tab, Val),
    OnLoadFun([{delete_object, Val}]),
    add_recs(Rest, OnLoadFun, N+1);
add_recs([{{Tab, Key}, Val, update_counter} | Rest], OnLoadFun, N) ->
    {RecName, Incr} = Val,
	...
	_ ->
	    ...
	    OnLoadFun([{write, Zero}])
    end,
    add_recs(Rest, OnLoadFun, N+1);
add_recs([LogH|Rest], OnLoadFun, N) 
  when record(LogH, log_header),  
       LogH#log_header.log_kind == dcl_log, 
       LogH#log_header.log_version >= "1.0" ->    
    add_recs(Rest, OnLoadFun, N);
add_recs([{{Tab, _Key}, _Val, clear_table} | Rest], OnLoadFun, N) ->
    true = traverse_delete(Tab, OnLoadFun),
%%%     true = ets:match_delete(Tab, '_'),
    add_recs(Rest, OnLoadFun, N+ets:info(Tab, size));  
add_recs([], _OnLoadFun, N) ->
    N.

traverse_delete(Tab, OnLoadFun) ->
    trav_delete(ets:select(Tab, [{'_',[],['$_']}], 50), OnLoadFun).

trav_delete({Recs, Cont}, OnLoadFun) ->
    OnLoadFun([{delete, Rec} || Rec <- Recs]),
    trav_delete(ets:select(Cont), OnLoadFun);
trav_delete('$end_of_table', _OnLoadFun) ->
    true.
